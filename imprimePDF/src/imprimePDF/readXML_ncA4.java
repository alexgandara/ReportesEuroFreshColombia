...
//		Q:\conector.global\data\20492896269\10_formatos
	...
	//		NodeList nList = doc.getElementsByTagName("Invoice");
			...
			String _temp=_file.substring(15,28);
	...
	//		System.out.println("Pais del Receptor_ _ _ _ _ _ _ : " + Cabecera.get_pais_receptor());
			...
			NodeList nList_PayableAmount_gra = doc.getElementsByTagName("cbc:PayableAmount");
			Node nNode_PayableAmount_gra = nList_PayableAmount_gra.item(0);
			Cabecera.set_total_gravado(Double.parseDouble(nNode_PayableAmount_gra.getTextContent()));
			System.out.println("Importe Gravado_ _ _ _ _ _ _: " + Cabecera.get_total_gravado());
			
			
			
			//cbc:PayableAmount subtotal cbc:PayableAmount inafecta
			NodeList nList_PayableAmount_ina = doc.getElementsByTagName("cbc:PayableAmount");
			Node nNode_PayableAmount_ina = nList_PayableAmount_ina.item(1);
			Cabecera.set_total_inafecto(Double.parseDouble(nNode_PayableAmount_ina.getTextContent()));
			System.out.println("Importe Inafecto _ _ _ _ _ _: " + Cabecera.get_total_inafecto());

			
			
			//cbc:PayableAmount subtotal cbc:PayableAmount exonerado
			NodeList nList_PayableAmount_exo = doc.getElementsByTagName("cbc:PayableAmount");
			Node nNode_PayableAmount_exo = nList_PayableAmount_exo.item(2);
			...
			Node nNode_PayableAmount_t = nList_PayableAmount_t.item(3);
			try {
	...
	//		Cabecera.set_total_letra(nNode_Value.getTextContent());
			...
			for (int _xx=0; _xx<=199;_xx++) {
				Detalle[_xx] = new factura_detalle();
			...
			for (int _xx=0; _xx<=199;_xx++) {
				Detalle_email[_xx] = new factura_detalle_email();
