...
public class readXMLGratis {
	...
	//		NodeList nList = doc.getElementsByTagName("Invoice");
			...
			// cbc:TaxableAmount
			NodeList nList_TaxableAmount_Det = doc.getElementsByTagName("cbc:TaxAmount");
	...
	//		Node nNode_StreetName = nList_StreetName.item(0);
			Cabecera.set_direccion_emisor("CAL.JULIAN ARCE NRO. 341 DPTO. B URB. SANTA CATALINA  LIMA - LIMA - LA VICTORIA");
			...
			NodeList nodeList_AccountingCustomerParty = doc.getElementsByTagName("cac:PartyIdentification").item(1).getChildNodes();
			Node nNode_AccountingCustomerParty = nodeList_AccountingCustomerParty.item(1);
			try {
				_RUC_RECEPTOR=nNode_AccountingCustomerParty.getTextContent().substring(0, 11);
			}  catch (Exception h) {
				_RUC_RECEPTOR="00000000";
			}
			...
			Node nNode_StreetName_r = nList_StreetName_r.item(0);
...
//				 System.out.println("Importe Gratuitos_ _ _ _ _ _ _: " + Cabecera.get_total_gratuitas());
			...
			Node nNode_TaxableAmount2 = nList_TaxableAmount2.item(3);
			Cabecera.set_total_gravado(Double.parseDouble(nNode_TaxableAmount2.getTextContent()));
		...
		//	Cabecera.set_total_igv(Double.parseDouble(nNode_TaxAmount.getTextContent()));
			...
			Node nNode_PayableAmount_t = nList_PayableAmount_t.item(0);
...
//			NodeList nList_linea = doc.getElementsByTagName("cbc:InvoicedQuantity");
	...
	//		NodeList nList_InvoicedQuantity = doc.getElementsByTagName("cbc:InvoicedQuantity");
			...
			//	try {
			//		Node Node_TaxableAmount_Det = nList_TaxableAmount_Det.item(_linea+5);
				//	double _igv_Det= Double.parseDouble(Node_TaxableAmount_Det.getTextContent());
					Detalle[_lineaArreglo].set_igv(_subtotal_sin_igv*.18);
	...
	//			}
			...
			for (int _linea_imp=0;_linea_imp<_total_linea;_linea_imp++) {
				...
				Detalle[_linea_imp].set_subtotal(Detalle[_linea_imp].get_precio_unitario()*Detalle[_linea_imp].get_cantidad()*1.18);
				Detalle[_linea_imp].set_subtotal_sin_igv(Detalle[_linea_imp].get_precio_unitario()*Detalle[_linea_imp].get_cantidad());
		...
		//		Cabecera.set_total(Cabecera.get_subtotal()+Detalle[_linea_imp].get_igv());
			...
			//factura.imp_factura(_file_xml, Cabecera, Detalle);
				...
				printPDFA4Gratis.imp_factura(_file_xml, Cabecera, Detalle, _lineas_Descripcion,_file_pdf, _file_jpg);
